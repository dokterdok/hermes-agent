...
class HostedRoomArtifactMixin:
    """Own room attachments, terminal artifact publication, and durable retries."""
    ...
    def _load_task_attachments(
        ...
        target_member_id = str(task.get("payload", {}).get("target_member_id") or "")
    ...
    def _publish_terminal_tasks(
        ...
        for status in ("deferred", "settled", "failed", "cancelled"):
            for task in driver.list_tasks(
                ...
                has_artifact_ack = isinstance(result, Mapping) and bool(
    ...
    def _import_terminal_artifacts(
        ...
        *,
        ...
        if existing_message is not None:
            existing_attachments = existing_message.get("payload", {}).get(
                "attachments"
            )
            if not isinstance(existing_attachments, list) or not existing_attachments:
                raise RuntimeError(
                    "room output artifact publication changed after commit"
                )
    ...
    def _retire_failed_terminal_artifacts(
        ...
        *,
        ...
        try:
            ...
            removed = (
                retirement.get("removed") if isinstance(retirement, Mapping) else None
            )
            if (
    ...
    def _discard_cancelled_task_artifacts(
        ...
        profile_root = (
            self.root if profile == "default" else self.root / "profiles" / profile
        )
        RoomArtifactOutbox(profile_root / "state.db").discard_durably(scope)
