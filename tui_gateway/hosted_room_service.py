...
from tui_gateway.hosted_room_artifact_service import (
    HostedRoomArtifactMixin,
    shutil as shutil,  # Preserve the existing Poppler probe patch path.
)
...
class HostedRoomService(HostedRoomArtifactMixin):
        ...
        *,
        ...
        try:
            ...
            for stored in stored_links:
                ...
                self._persisted_peer_route_keys.add((stored.room_id, stored.member_id))
    ...
    def _hydrate_persisted_peer_route(
        ...
        with self._policy_lock:
            ...
            if (
                ...
                and not isinstance(client, PeerRunsHTTPClient)
            ):
                return route, client
            try:
    ...
    def _recover_peer_admission(
        ...
        if not isinstance(prompt, str) or source_event_seq < 1 or not route.trace_id:
            raise RuntimeError(
                "peer room admission identity is unavailable for recovery"
            )
        attachment_manifest = []
        attachment_payloads = []
    ...
    def _set_pending_action(
        ...
        if stored_action is not None and stored_action.get("kind") in {
            ...
            reported_epoch = int(
                stored_action.get("authority_epoch") or room.get("authority_epoch") or 0
            )
            ...
            if reported_gateway_id != str(
                ...
                approvals.clear_pending_approval(
                    ...
                    room_id=room_id,
                    member_id=member_id,
                    request_id=(None if is_clear else stored_action.get("request_id")),
                ...
                with self._policy_lock:
                    ...
                    same_authority = (
                        str((current or {}).get("authority_gateway_id") or "")
                        == reported_gateway_id
                        and int((current or {}).get("authority_epoch") or 0)
                        == reported_epoch
                    )
    ...
    def _apply_pending_control_approvals(
        ...
        for command in pending:
            ...
            if coordinates not in durable:
                ...
                continue
            with self._policy_lock:
                action = self._pending_actions.get((binding.room_id, coordinates[2]))
            if (
            ...
            try:
                ...
                result = "Approved once." if command["choice"] == "once" else "Denied."
    ...
    def approve_room_task(
        ...
        *,
        ...
        if str((action or {}).get("authority_gateway_id") or "") != str(
            room["authority_gateway_id"]
        ) or int((action or {}).get("authority_epoch") or 0) != int(
