  ...
  try {
    ...
    for (const route of routes) {
      ...
      for (const listedRaw of listedRooms) {
        ...
        hostedRoomControlEnrollment.schedule(listedRoom, route, routesByConnection, capabilities)
...
export async function createAutonomousHostedGroupChat({
  ...
  const controls = hostedRoomControlEnrollment.forCreation(homeRoute, roomId, probe.capabilities)
  ...
  try {
    ...
    for (const [index, item] of members.entries()) {
      ...
      controls.add(memberId, item.member, connectionId, catalog.installation_id, invitation.target_profile)
    ...
    for (const registration of peerRegistrations) {
      await controls.enroll(registration.member_id)
...
export function stopHostedRoomRuntime() {
  ...
  stopHostedRoomCleanup()
  hostedRoomControlEnrollment.reset(false)
