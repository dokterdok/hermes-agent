"""#98073 Group Chat commands on canonical owner/Home consent."""
import hashlib
import json
import time
...
_MUTATIONS = frozenset({'stop', 'retry', 'discard', 'confirm'})
...
class GroupChatSlashCommandsMixin:
    ...
    def _group_chat_help(command):
        return '\n'.join(['**Group Chats**', '', f'`{command}` - Choose a Group Chat.',
            ...
            f'`{command} 7 permissions` - Manage remembered permissions.', '',
    ...
    async def _handle_rooms_command(self, event):
        ...
        try:
            ...
            if len(words) > 1 and words[0].isdecimal():
                ...
                if kind in {'remember', 'permissions', 'forget', 'approvals', 'approve', 'deny'}:
                    from gateway.group_chat_decisions import dispatch_decision_command
                    return await dispatch_decision_command(self, event, backend, room, kind, argument, command, stamp)
