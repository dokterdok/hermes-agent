...
def _current(connection, ref, epoch):
    ...
    if live.source is None or live.source.platform != Platform.LOCAL:
        raise _unavailable()
...
def _snapshot(connection, ref, params, epoch):
    ...
    with _readonly(path) as conn:
        ...
        if retired is not None and retired[0] >= scope.execution_generation:
            raise _unavailable()
        artifact = conn.execute('SELECT * FROM hosted_room_output_artifacts WHERE artifact_id=? AND scope_key=?',
