...
class HomeChannel:
    ...
    selection_id: Optional[str] = None  # Opaque generation of an explicit Home selection.
    group_audience_ack: Optional[str] = None  # Audience fingerprint, not command authority.

    def to_dict(self) -> Dict[str, Any]:
        optional = {k: v for k in ("thread_id", "user_id", "scope_id", "selection_id", "group_audience_ack")
                    if (v := getattr(self, k))}
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "HomeChannel":
        ...
        optional.update({k: data.get(k) if isinstance(data.get(k), str) else None
