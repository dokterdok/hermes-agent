...
The invitation→NEW hosted-room path calls this after a task has settled and
primary terminal evidence already exists. History and info do not. Primary
``publish_terminal`` does not. Send-consent is not passed and is not
publication authority. A missing contract fails closed before any secondary write.
"""
...
def call_settled_invitation_secondary(service, binding, task):
    ...
    Returns None when this task is not that publication. Does not insert rows
