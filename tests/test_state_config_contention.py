...
def test_existing_database_does_not_wait_for_config_writer(
    ...
    def opener():
        ...
        try:
            ...
            assert apply_wal_with_fallback(
                conn, db_label=f"contention-{mode}-{require_wal}-{vulnerable}",
                require_wal=require_wal,
            ) == selected
            assert conn.execute("PRAGMA journal_mode").fetchone()[0] == selected
            if selected == "wal":
                assert conn.execute("PRAGMA journal_size_limit").fetchone()[0] == wal._WAL_SIZE_LIMIT_BYTES
    ...
    if vulnerable:
        ...
        expected = "already in WAL mode" if mode == "wal" else "using journal_mode=DELETE"
