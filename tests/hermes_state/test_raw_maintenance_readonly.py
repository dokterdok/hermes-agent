"""Legacy read-only maintenance previews omit absent ledgers only (29b11c1d4330).

No preview may repair the SQLite file; strict writers still require both ledgers.
"""
import sqlite3
...
from tests.hermes_state.test_raw_history_ledger import _ended, _ledger
...
def test_readonly_preview_checks_present_ledger_without_mutating_file(tmp_path, preview, schema):
    ...
    with sqlite3.connect(path) as conn:
        ...
        if schema in {'admissions', 'uppercase', 'query-denied'}:
            _ledger(conn, 'protected', 'admission-terminal')
            ...
            _ledger(conn, 'protected', 'worker-terminal')
